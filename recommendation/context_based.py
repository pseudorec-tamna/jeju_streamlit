...
def context_based_recommendation(start_location, transition_matrix_df, shinhan_df):
    # 다른 추천 결과의 rec['MCT_NM']을 기준으로 next visit 제안하기
    # query를 id로 받는 방식 -> 카카오맵API?
    if start_location in transition_matrix_df.index:
        shinhan_ids = transition_matrix_df.index

    top_3_shinhan_poi = get_top_3_end_locations(start_location, transition_matrix_df,top_k=3,in_shin = 'Y')
    result_id = [x[0] for x in top_3_shinhan_poi]
    rec = shinhan_df[shinhan_df.id.isin(result_id)][[
